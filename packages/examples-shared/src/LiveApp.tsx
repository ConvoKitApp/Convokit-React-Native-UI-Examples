...
function LiveInbox({ client, pickAttachment, onError }: {
  ...
  const markUnread = () => {
    ...
    // controller's `error`, which the default list renders inline with `Retry`, and rejects, so it is
    // not escalated here; only a rejection the list did not report replaces the screen: the
